...
class Controlador:
    ...
    def configurar_eventos(self):
        self.visao.tipo_figura.trace_add("write", self.aparecer_menu_poligonos)
    ...
    def aparecer_menu_poligonos(self, *args):
        if self.visao.tipo_figura.get() == "Polígono":
            self.visao.mostrar_menu_poligono()
        else:
            self.visao.esconder_menu_poligono()
    ...
    def desenhar_poligono(self, event):
        ...
        total = int(self.visao.menu_poly.get()) * 2

        if len(self.modelo.pontos_poligonos) < 6:
            return

        figura = Poligono(
            self.visao.cor_borda.get(),
            float(self.visao.espessura.get()),
            self.visao.cor_preenchimento.get(),
            self.modelo.pontos_poligonos.copy()
